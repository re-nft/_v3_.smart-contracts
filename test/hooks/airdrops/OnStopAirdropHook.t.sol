...
import {OnStopAirdropHook} from "@src/examples/airdrop-hooks/OnStopAirdropHook.sol";
import {
    Hook,
    OrderType,
    OrderMetadata,
...
contract Hook_OnStopAirdropHook_Test is BaseTest {
    // hook contract
    OnStopAirdropHook public hook;
    ...
    function setUp() public override {
        ...
        hook = new OnStopAirdropHook(
            address(stop),
            address(erc721s[0]),
            address(tokenToMint)
        );

        // admin enables the hook. Use binary 00000100 so that the hook
        // is enabled for `onStop` calls only
        vm.prank(deployer.addr);
        guard.updateHookStatus(address(hook), uint8(4));

        // assert that hook set up was successful
        assertEq(STORE.hookOnStart(address(hook)), false);
        assertEq(STORE.hookOnStop(address(hook)), true);
        assertEq(STORE.hookOnTransaction(address(hook)), false);
    }

    function test_Success_TokenMintedOnStop() public {
        // create a BASE order
        createOrder({
            offerer: alice,
        ...
        // assert that the token has not yet been minted to bob
        assertEq(tokenToMint.totalSupply(), 0);
